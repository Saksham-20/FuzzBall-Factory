...
## Data layer (mock by default, real API behind a flag)
All data goes through `web/src/lib/api/*.ts` (async functions, simulated latency, localStorage DB in `web/src/lib/mock/db.ts`). **Pages must only call these functions**, never touch `db` directly, so the swap to the real NestJS API is config-only: with `NEXT_PUBLIC_USE_MOCK=false` each function calls its twin in `lib/api/real/*` (a few server components still import `lib/mock/catalog` directly; see `TODOS.md`). Available modules: `auth`, `catalog`, `orders`, `account`, `custom`, `admin`, `shipping`, `settings`, `reviews`, `uploads`, `checkout-extra`. Read the exported signatures before you use them. Totals come from `lib/pricing.ts` via `orders.quote()`; status labels/stamps from `lib/status.ts`; formatting from `lib/format.ts`; WhatsApp links from `lib/whatsapp.ts`.
