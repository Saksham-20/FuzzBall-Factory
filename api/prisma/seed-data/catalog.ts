/*
 * PLACEHOLDER(catalogue): SAMPLE data mirrored from web/src/lib/mock/catalog.ts (same categories, same 15
 * products, same /samples/*.jpg image paths served by the web app). None of it is the maker's real stock,
 * prices or lead times. Every product is seeded with sample=true. Keep in sync with the web mock until the
 * real catalogue is entered through the admin.
 */
...
export interface Product {
  id: string; slug: string; batch: number; name: string; tagline: string; description: string; category: string;
  price: number; compareAtPrice?: number; fulfilment: Fulfilment; leadTimeDays: number; fiber: string; sizeCm: string;
  weightG: number; care: string[]; images: { src: string; alt: string }[]; swatches: Swatch[];
  variants: { id: string; colour: string; size?: string; priceDelta: number; stock: number }[];
  isOneOfAKind: boolean; customizable: boolean; giftable: boolean; occasions: string[]; tags: string[];
  status: 'DRAFT' | 'PUBLISHED' | 'ARCHIVED'; sample?: boolean; createdAt: string;
}
